...
def search_local_tickers(query):
    ...
    # 원본 검색어 자체가 영문 2글자 이하인 경우(예: lg, sk)에는 토큰 분리 후 소실되지 않도록 1글자 필터링 우회
    original_compact = normalize_match_text(query)
...
def calculate_signal_monthly_stats(strategy_df, is_portfolio=False):
...
def run_stochastic_backtest(df, k_period, d_period, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False):
    ...
    stoch_df['%K'] = ((stoch_df['종가'] - low_min) / (high_max - low_min).replace(0, 1e-10)) * 100
...
def run_adx_dmi_backtest(df, period, adx_threshold, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False):
    ...
    adx_df['+DI'] = 100 * plus_dm.rolling(period).mean() / atr.replace(0, 1e-10)
    adx_df['-DI'] = 100 * minus_dm.rolling(period).mean() / atr.replace(0, 1e-10)
    dx = ((adx_df['+DI'] - adx_df['-DI']).abs() / (adx_df['+DI'] + adx_df['-DI']).replace(0, 1e-10)) * 100
...
# 국내 계열사 매칭 결과가 강제로 삭제되지 않도록 우회 조건을 마련합니다.
is_group_name_search = normalize_match_text(ticker_input) in ["lg", "sk", "gs", "cj", "hd"]

if is_ticker_like_input(ticker_input) and not is_group_name_search:
