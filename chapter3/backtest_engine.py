...
def run_generic_backtest_with_defense(
    df,
    buy_signals,
    initial_budget,
    fee_rate_pct,
    ...
    if defense_df is not None and defensive_mode == "방어자산":
        ...
        defense_div_yield = (def_div / def_close.shift(1).fillna(def_close)).fillna(0.0)
        defense_return = (def_close / def_close.shift(1).fillna(def_close)).fillna(1.0) + (defense_div_yield if use_drip else 0.0)
    else:
    ...
    # Asset return
    attack_div_yield = (backtest_df['배당금'] / backtest_df['종가'].shift(1).fillna(backtest_df['종가'])).fillna(0.0) if '배당금' in backtest_df.columns else 0.0
    attack_return = (backtest_df['종가'] / backtest_df['종가'].shift(1).fillna(backtest_df['종가'])).fillna(1.0) + (attack_div_yield if use_drip else 0.0)
...
def run_vbt_backtest(df, K, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False, defense_df=None, defensive_mode="현금", attack_label="ATTACK", defense_label="DEFENSE"):
...
def run_ma_cross_backtest(df, short_period, long_period, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False, defense_df=None, defensive_mode="현금", attack_label="ATTACK", defense_label="DEFENSE"):
...
def run_rsi_backtest(df, period, buy_rsi, sell_rsi, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False, defense_df=None, defensive_mode="현금", attack_label="ATTACK", defense_label="DEFENSE"):
...
def run_bollinger_backtest(df, period, std_dev, initial_budget, fee_rate_pct, slippage_rate_pct, use_drip=False, defense_df=None, defensive_mode="현금", attack_label="ATTACK", defense_label="DEFENSE"):
...
def run_macd_backtest(df, fast_period=12, slow_period=26, signal_period=9, initial_budget=10000000, fee_rate_pct=0.15, slippage_rate_pct=0.10, use_drip=False, defense_df=None, defensive_mode="현금", attack_label="ATTACK", defense_label="DEFENSE"):
