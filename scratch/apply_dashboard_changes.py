...
# 3. Add DCA sidebar parameters
target_sidebar = 'elif strategy_choice == "스토캐스틱 오실레이터 전략":'
replacement_sidebar = """elif strategy_choice == "적립식 존버 물타기 (DCA) 전략":
...
# Apply modifications
mods = [
    (target_imports, replacement_imports),
    (target_selectbox, replacement_selectbox),
    (target_sidebar, replacement_sidebar),
    (target_desc, replacement_desc),
    (target_runner, replacement_runner),
    (target_label, replacement_label),
    (target_charts, replacement_charts),
    (target_returns, replacement_returns),
    (target_stats, replacement_stats),
    (target_columns, replacement_columns)
]

modified = True
for target, replacement in mods:
...
if modified:
    ...
    print("Could not apply all dashboard changes.")
